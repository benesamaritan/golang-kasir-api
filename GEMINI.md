...
The project leverages `devbox` for environment management and containerization, ensuring a consistent development and deployment experience. It also incorporates enhanced load testing using `k6` to verify the application's performance, health, and responsiveness across multiple endpoints.
...
2.  **Run the Application:**
    This command starts the application services defined in `process-compose.yaml`.
    ```sh
    devbox run kasir-up
    ```
    Alternatively, you can use:
    ```sh
    devbox services up kasir
    ```
    The application will be accessible on the port defined in your configuration (default `8000`).
